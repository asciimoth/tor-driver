...
UID and requires launch to reject it. A non-root helper starts with ambient
`CAP_NET_ADMIN`, launches a child through the native adapter, and proves that
the child gets no ambient or effective capabilities while the parent capability
is restored.
...
| Operational diagnostics | Bounded outgoing and shutdown stages, retained recent events, typed startup stages, and Tor-log redaction for startup and runtime bridge fields, credentials, executable paths, and onion key tokens. Split and delayed log lines retain every known bridge secret for the process lifetime. |
...
| Runtime bridges | Ordered network disable, all-or-nothing typed replacement, rollback after enable failure, proof that rejected bridge mode does not re-enable direct guards, and concurrent typed-event publication during repeated replacement. The private-network gate repeatedly changes a live client between direct and obfs4 modes under the race detector. |
| Onion services | Expanded-key conversion and text formats, injected persistence and storage failure cleanup, key erasure on service, Driver, and terminal-process closure, typed host/client authorization, descriptor waits/events, port removal/reopen and rollback, stream-limit policy, concurrent Driver close and drain, and close subscriptions. |
| Linux direct adapters | `openat2` symlink rejection, fd-relative removal, private modes, pidfd/process cleanup, ambient-capability removal and caller restoration, root/non-root identity decisions, fallback reporting, parent-migration rejection for caller and configured child identities, exact cgroup-empty checks, and generated cgroup/nftables rules for both IP families. |
