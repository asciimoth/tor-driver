...
docker run \
    ...
    --user root \
    "$image" \
    test -race -count=1 -tags=e2e -run '^(TestTorContainedTransportSocketDenial|TestTorContainedRejectsEscapableDelegation|TestTorBestEffortEnvironment)$' -v -timeout 3m .
